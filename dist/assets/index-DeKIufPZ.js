(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function i(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=i(n);fetch(n.href,a)}})();const jt=[{id:"priya",name:"Priya Kumar",email:"priya@contoso.com",role:"user",initials:"PK",color:"#0C447C",navAccess:["portal","myreqs","chat"]},{id:"sanjay",name:"Sanjay Kumar",email:"sanjay@contoso.com",role:"manager",initials:"SK",color:"#3C3489",navAccess:["approvals","portal","myreqs","chat"],pendingApprovals:3},{id:"chen",name:"Chen Wei",email:"chen@contoso.com",role:"admin",initials:"CW",color:"#633806",navAccess:["dashboard","requests","security","zerotrust","privaccts","m365config","licenses","agents","msgcenter","applications","intune","portal","myreqs","chat","audit","settings"]},{id:"aisha",name:"Aisha Raza",email:"aisha@contoso.com",role:"super",initials:"AR",color:"#791F1F",navAccess:["dashboard","requests","security","zerotrust","privaccts","m365config","licenses","agents","msgcenter","applications","intune","portal","myreqs","chat","audit","settings","graphapi","sso"]}],hi={auth:{clientId:"04d3be8d-d433-4367-893e-eccc82190a11",authority:"https://login.microsoftonline.com/b9cc8284-05ed-452f-877a-970779430dcb",redirectUri:window.location.origin+"/callback"},cache:{cacheLocation:"sessionStorage",storeAuthStateInCookie:!1},system:{allowNativeBroker:!1}},fi={scopes:["openid","profile","email","User.Read"]};let N=null;async function bi(){try{if(!window.msal)return console.warn("MSAL not loaded yet"),null;N=new window.msal.PublicClientApplication(hi),await N.initialize();const e=await N.handleRedirectPromise();if(e)return console.log("✓ Authenticated from redirect:",e.account.name),e.account;const t=N.getAllAccounts();return t.length>0?(console.log("✓ Already authenticated as:",t[0].name),t[0]):null}catch(e){return console.error("MSAL init error:",e.message),null}}async function xi(){try{if(!N)return console.error("MSAL not initialized"),null;const e=await N.loginPopup(fi);return console.log("✓ Login successful:",e.account.name),e.account}catch(e){return e.errorCode==="user_cancelled"?console.log("User cancelled login"):console.error("Login error:",e.errorCode,e.errorMessage),null}}async function wi(){try{if(!N)return;const e={account:N.getAllAccounts()[0]};await N.logout(e),console.log("✓ Logged out")}catch(e){console.error("Logout error:",e)}}function Ai(){const e=document.getElementById("app-header"),t=d.currentUser;t&&(e.innerHTML=`
  ...
  `,Ni(e)}function _e(e){return e==="approved"?'<span class="badge success dot">Approved</span>':e==="overdue"?'<span class="badge danger dot">Overdue</span>':e==="in-progress"?'<span class="badge info dot">In Progress</span>':e==="rejected"?'<span class="badge neutral dot">Rejected</span>':'<span class="badge warning dot">Pending</span>'}function Gi(e){const t=e.status==="pending"||e.status==="overdue"||e.status==="in-progress";return`
    <tr data-id="${e.id}">
...
**Graph API:** GET /beta/identity/conditionalAccess/policies`}];let Ke=te,X="executive",h={priority:"all",category:"all",status:"all"},Ce="7d",ce=[],vt=!1;const Fi=[{id:"executive",label:"Executive",icon:"ti-layout-dashboard"},{id:"securescore",label:"Secure Score",icon:"ti-shield-check"},{id:"identity",label:"Identity",icon:"ti-user-check"},{id:"email",label:"Email",icon:"ti-mail"},{id:"endpoint",label:"Endpoint",icon:"ti-device-laptop"},{id:"teams",label:"Teams",icon:"ti-brand-teams"},{id:"sharepoint",label:"SharePoint",icon:"ti-brand-sharepoint"},{id:"dataprotection",label:"Data Protection",icon:"ti-lock"},{id:"privaccess",label:"Priv. Access",icon:"ti-crown"},{id:"guests",label:"Guests",icon:"ti-user-plus"},{id:"incidents",label:"Incidents",icon:"ti-alert-triangle"},{id:"recommendations",label:"Recommendations",icon:"ti-checklist"},{id:"copilot",label:"Security Copilot",icon:"ti-robot"},{id:"apiref",label:"API Reference",icon:"ti-api"}];async function Ui(){const e=document.getElementById("page-security");if(e){try{console.log("📡 Fetching real security data from backend...");const t=await Wt();t.success&&(Ke=t.data||te,console.log("✅ Loaded real secure score from API"))}catch(t){console.warn("⚠️ Using simulated secure score:",t.message),Ke=te}L(e)}}function L(e){var s,n;const t=b.filter(a=>a.severity==="critical").length;b.filter(a=>a.severity==="high"&&a.status!=="resolved").length;const i=I.filter(a=>a.priority==="critical"||a.priority==="high").length;e.innerHTML=`
  ...
  </svg>`}function zi(e,t=24){const i=Math.max(...e),s=Math.min(...e),n=i-s||1;return`<div style="display:flex;align-items:flex-end;gap:2px;height:${t}px">
  ...
  </div>`}function ji(e,t){return e==="pass"||e===!0?`<span style="color:var(--clr-success-text)"><i class="ti ti-circle-check"></i> ${t}</span>`:e==="partial"||e==="warn"?`<span style="color:var(--clr-warning-text)"><i class="ti ti-alert-triangle"></i> ${t}</span>`:`<span style="color:var(--clr-danger-text)"><i class="ti ti-circle-x"></i> ${t}</span>`}function Hi(){return({executive:gt,securescore:Wi,identity:_i,email:Vi,endpoint:Ji,teams:Ki,sharepoint:Yi,dataprotection:Qi,privaccess:Xi,guests:Zi,incidents:es,recommendations:ts,copilot:is,apiref:ss}[X]||gt)()}function gt(){const e=te;return`
    <!-- Secondary KPI row -->
    ...
    <div class="grid-2 mb-3" style="gap:16px">
      ...
      <div class="card">
        ...
        <div style="margin-bottom:12px" id="exec-trend-chart">
          ${zi(Ce==="30d"?e.trend30d:e.trend7d,32)}
        </div>
      ...
      <div class="card">
        ...
        <div class="sec-svc-grid">
          ${[{name:"Identity",icon:"ti-user-check",score:68,color:"#0C447C",bg:"#E6F1FB",issues:$.highRiskUsers},{name:"Email",icon:"ti-mail",score:71,color:"#854F0B",bg:"#FAEEDA",issues:_t.externalForwardingRules+1},{name:"Endpoint",icon:"ti-device-laptop",score:58,color:"#3B6D11",bg:"#EAF3DE",issues:ye.vulnerable},{name:"Teams",icon:"ti-brand-teams",score:74,color:"#3C3489",bg:"#EEEDFE",issues:Vt.publicTeams},{name:"SharePoint",icon:"ti-brand-sharepoint",score:66,color:"#3B6D11",bg:"#EAF3DE",issues:Jt.anonymousLinks},{name:"Data",icon:"ti-database",score:61,color:"#3C3489",bg:"#EEEDFE",issues:Ve.dlpViolations30d},{name:"Priv Access",icon:"ti-crown",score:78,color:"#854F0B",bg:"#FAEEDA",issues:Kt.permanentAssignments},{name:"Guests",icon:"ti-user-plus",score:72,color:"#633806",bg:"#FAEEDA",issues:Je.dormantGuests90d}].map(t=>(t.score>=80||t.score>=65,`<div class="sec-svc-tile" data-goto="${t.name.toLowerCase().replace(" ","").replace(".","")}" style="cursor:pointer">
              ${Pe(t.score,100,40)}
              <div style="flex:1;min-width:0">
  ...
  `}function _i(){const e=$;return`
    ...
    <div class="grid-2 mb-3" style="gap:16px">
      ...
      <div class="card">
        ...
        ${j(["Require phishing-resistant MFA (FIDO2/CBA) for all admins","Remove legacy authentication via dedicated CA policy","Reduce Global Admin count to maximum 2 PIM-protected accounts","Review 18 CA policy exclusions — remove unnecessary exemptions"])}
      </div>
    </div>
  `}function Vi(){const e=_t;return`
    <div class="kpi-row mb-3">
    ...
    <div class="grid-2 mb-3" style="gap:16px">
      ...
      <div class="card">
        ...
        ${j(["Enable Strict Preset Security Policies in Defender for Office 365","Disable automatic external mail forwarding tenant-wide","Upgrade DMARC policy from quarantine to reject","Extend Safe Attachments coverage to all users (currently partial)"])}
      </div>
    </div>
  `}function Ji(){const e=ye;return`
    <div class="kpi-row mb-3">
    ...
    <div class="grid-2 mb-3" style="gap:16px">
      ...
      <div class="card">
        ...
        ${j(["Patch 23 devices missing critical security updates","Isolate ransomware-affected device MBX-LAPTOP-047","Enable BitLocker on remaining 36 unencrypted devices","Harden SMB and RDP access on Windows 10 devices"])}
      </div>
    </div>
  `}function Ki(){const e=Vt;return`
    <div class="kpi-row mb-3">
    ...
    <div class="grid-2" style="gap:16px">
      ...
      <div class="card">
        ...
        ${j(["Archive 23 inactive Teams (90d+) to reduce sprawl and exposure","Assign owners to 5 unowned Teams","Conduct guest access review for 34 guest-enabled Teams","Review 8 public Teams — consider making private","Restrict external domains to known partners only"])}
      </div>
    </div>
  `}function Yi(){const e=Jt;return`
    <div class="kpi-row mb-3">
    ...
    <div class="grid-2" style="gap:16px">
      ...
      <div class="card">
        <div class="card-title mb-3"><i class="ti ti-shield"></i> Recommendations</div>
        ${j(["Remove 3 anonymous sharing links — replace with authenticated sharing","Review 5 overshared sites with > 100 members","Enable sensitivity labels for automatic file classification",'Restrict external sharing to "Existing guests only" on high-risk sites',"Configure DLP policy for SharePoint to reach 100% coverage"])}
      </div>
    </div>
  `}function Qi(){const e=Ve;return`
    <div class="kpi-row mb-3">
  ...
  `}function Xi(){const e=Kt;return`
    ...
    <div class="grid-2" style="gap:16px">
      ...
      <div class="card">
        ...
        ${j(["Convert 4 permanent admin role assignments to PIM eligible","Implement Just-in-Time access for all privileged roles","Conduct quarterly access review for all admin role holders","Enable PIM access review notifications for approvers"])}
  ...
  `}function Zi(){const e=Je;return`
    ...
    <div class="grid-2" style="gap:16px">
      ...
      <div class="card">
        <div class="card-title mb-3"><i class="ti ti-shield"></i> Recommendations</div>
        ${j(["Remove 3 expired guest accounts immediately","Review and remove 12 dormant guests (90d+ no sign-in)","Schedule overdue quarterly access review for 14 guests","Require manager attestation for all guest renewals","Implement automatic expiry policy (365 days max)"])}
      </div>
    </div>
  `}function es(){const e=b.filter(r=>r.status!=="resolved"),t=b.filter(r=>r.status==="resolved"),i=b.filter(r=>r.severity==="critical").length,s=b.filter(r=>r.severity==="high").length,n=b.filter(r=>r.severity==="medium").length,a=b.filter(r=>r.severity==="low").length;return`
    <div class="kpi-row mb-3">
    ...
    ${t.length>0?`
      ...
      `).join("")}
    `:""}
  `}function ts(){const e=I.filter(s=>!(h.priority!=="all"&&s.priority!==h.priority||h.category!=="all"&&s.category!==h.category||h.status!=="all"&&s.status!==h.status)),t=e.reduce((s,n)=>s+n.scoreGain,0),i=[...new Set(I.map(s=>s.category))];return`
  ...
  `}function is(){(!vt||ce.length===0)&&(ce=[{role:"ai",text:`**M365 Security Copilot** — I have full context of your security posture across all 15 data sources.
      ...
      <div style="overflow-y:auto;flex:1;padding-bottom:8px" id="sec-copilot-msgs">
        ${ce.map(t=>`
          <div class="chat-msg ${t.role==="ai"?"ai":"user-msg"}" style="max-width:85%;margin-bottom:12px">
            ...
            <div class="chat-bubble">${Yt(t.text)}</div>
          </div>
        `).join("")}
      </div>
...
Current status: Secure Score 64/95 · ${b.filter(o=>o.status!=="resolved").length} active incidents · ${$.highRiskUsers} high-risk users.`;ce.push({role:"ai",text:r}),s&&(s.innerHTML+=`<div class="chat-msg ai" style="max-width:85%;margin-bottom:12px">
    ...
    `).join("")}
  </div>`}function j(e){return`<div style="margin-top:12px">
  ...
  `,rs(e),e.querySelector("#zt-rescan").addEventListener("click",()=>{const l=e.querySelector("#zt-rescan");l.innerHTML='<span class="spinner dark"></span> Scanning...',l.disabled=!0,setTimeout(()=>{l.innerHTML='<i class="ti ti-refresh"></i> Re-scan',l.disabled=!1,p("Zero Trust scan complete — 12 controls assessed.","success")},2e3)})}function rs(e){const t=e.querySelector("#zt-pillars");t&&(t.innerHTML=Qt.map((i,s)=>{const n=i.controls.filter(l=>l.status==="pass").length,a=i.controls.filter(l=>l.status==="warn").length,r=i.controls.filter(l=>l.status==="fail").length,o=i.controls.length,c=r>0?"danger":a>0?"warning":"success";return`
      <div class="card mb-3" style="padding:0;overflow:hidden">
  ...
  `;const s=e.querySelector("#cfg-topic-grid");ke.forEach(n=>{const a=at(n),r=Ye(a.score),o=document.createElement("div");o.className="cfg-topic-card";const c=Re[n.id]||{bg:"#f0f0f0",color:"#555"};o.innerHTML=`
    ...
    `,o.addEventListener("click",()=>Zt(e,n)),s.appendChild(o)}),e.querySelector("#cfg-scan-now").addEventListener("click",()=>{const n=e.querySelector("#cfg-scan-now");n.innerHTML='<span class="spinner dark"></span> Scanning...',n.disabled=!0,setTimeout(()=>{n.innerHTML='<i class="ti ti-refresh"></i> Run scan now',n.disabled=!1,p("Scan complete — 96 controls validated.","success")},2200)}),e.querySelector("#cfg-agent-btn").addEventListener("click",()=>vs(e))}let f={search:"",status:"all",profile:"all"};function Zt(e,t){ie="topic",Me=t,f={search:"",status:"all",profile:"all"};const i=at(t),s=Ye(i.score),n=Re[t.id]||{bg:"#f0f0f0",color:"#555"};e.innerHTML=`
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:10px">
  ...
  `,e.querySelector("#cfg-back").addEventListener("click",()=>Le(e)),e.querySelector("#cfg-topic-settings-btn").addEventListener("click",async()=>await x("settings"));const a=e.querySelector("#cfg-topic-settings-link");a&&a.addEventListener("click",async r=>{r.preventDefault(),await x("settings")}),e.querySelector("#cfg-topic-scan").addEventListener("click",()=>{const r=e.querySelector("#cfg-topic-scan");r.innerHTML='<span class="spinner dark"></span> Scanning...',r.disabled=!0,setTimeout(()=>{r.innerHTML='<i class="ti ti-refresh"></i> Scan topic',r.disabled=!1,p(`Topic ${t.num} scan complete.`,"success")},1800)}),e.querySelector("#cfg-filter-search").addEventListener("input",r=>{f.search=r.target.value,J(e,t)}),e.querySelector("#cfg-filter-status").addEventListener("change",r=>{f.status=r.target.value,J(e,t)}),e.querySelector("#cfg-filter-profile").addEventListener("change",r=>{f.profile=r.target.value,J(e,t)}),J(e,t)}function J(e,t){const i=e.querySelector("#cfg-controls-area");i&&(i.innerHTML=t.subsections.map(s=>{const n=s.controls.filter(a=>{const r=he(a);if(f.status!=="all"&&r!==f.status||f.profile!=="all"&&a.profile!==f.profile)return!1;if(f.search){const o=f.search.toLowerCase();if(!a.id.toLowerCase().includes(o)&&!a.title.toLowerCase().includes(o))return!1}return!0});return n.length===0?"":`
    ...
    `);let n="";return d.settings.showPSCommands&&e.ps&&(n+=`
      <div class="code-block-header">
        ...
        <button class="btn btn-xs cfg-copy-btn" data-code="${ms(e.ps)}"><i class="ti ti-copy"></i> Copy</button>
      </div>
  ...
  `}const mt=Object.fromEntries(ke.map(e=>[e.id,!0]));function vs(e){ie="agent";const t=d.cfgAgentLog;e.innerHTML=`
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:10px">
    ...
    <div class="card">
      ...
      <div id="agent-scan-log" style="max-height:300px;overflow-y:auto">
        ${t.length===0?'<div class="empty-state">No scan log entries yet. Run a scan to generate log entries.</div>':t.map(ei).join("")}
      </div>
    </div>
  `,e.querySelector("#cfg-agent-back").addEventListener("click",()=>Le(e)),e.querySelector("#agent-save-cfg").addEventListener("click",()=>{d.settings.agentSchedule=e.querySelector("#agent-schedule").value,d.settings.agentAlertEmail=e.querySelector("#agent-email").value,d.settings.agentAlertOnFail=e.querySelector("#agent-alert-fail").checked,d.settings.agentDailyDigest=e.querySelector("#agent-daily-digest").checked,m(),p("Agent configuration saved.","success")}),e.querySelector("#agent-clear-log").addEventListener("click",()=>{d.cfgAgentLog=[],m(),e.querySelector("#agent-scan-log").innerHTML='<div class="empty-state">Log cleared.</div>'}),e.querySelector("#cfg-agent-run-now").addEventListener("click",()=>{gs(e)}),e.querySelectorAll(".topic-enabled-toggle").forEach(i=>{i.addEventListener("change",s=>{mt[s.target.dataset.tid]=s.target.checked})})}function ei(e){const t={scan:"ti-scan",pass:"ti-circle-check",fail:"ti-circle-x",warn:"ti-alert-triangle",info:"ti-info-circle",done:"ti-check"},i={scan:"var(--clr-info-text)",pass:"var(--clr-success-text)",fail:"var(--clr-danger-text)",warn:"var(--clr-warning-text)",info:"var(--clr-info-text)",done:"var(--clr-success-text)"};return`
  ...
  `,xs(e),ws(e),As(e),e.querySelectorAll("#pa-tabs .tab-btn").forEach(t=>{t.addEventListener("click",()=>{e.querySelectorAll("#pa-tabs .tab-btn").forEach(i=>i.classList.remove("active")),e.querySelectorAll(".tab-panel").forEach(i=>i.classList.remove("active")),t.classList.add("active"),e.querySelector(`#pa-tab-${t.dataset.tab}`).classList.add("active")})}),e.querySelector("#pa-sync").addEventListener("click",()=>{const t=e.querySelector("#pa-sync");t.innerHTML='<span class="spinner dark"></span> Syncing...',t.disabled=!0,setTimeout(()=>{t.innerHTML='<i class="ti ti-refresh"></i> Sync tenant',t.disabled=!1,p("Tenant sync complete — 14 accounts updated.","success")},2e3)}),e.querySelector("#pa-tag-account").addEventListener("click",()=>{p("Tag account: select an account from the table below.","info")}))}function ft(e){return e==="High"?'<span class="badge danger dot">High</span>':e==="Medium"?'<span class="badge warning dot">Medium</span>':'<span class="badge neutral dot">None</span>'}function fs(e){return!e||e.length===0?'<span class="pa-mfa-pill none">No MFA</span>':e.map(t=>t==="SMS"?'<span class="pa-mfa-pill sms">SMS</span>':`<span class="pa-mfa-pill">${t}</span>`).join("")}function bs(e){return`<span class="pa-role-chip ${e.toLowerCase().includes("global")?"global":""}">${e}</span>`}function xs(e){var n,a;const t=e.querySelector("#pa-tab-accounts"),i="pa-acct-search";let s=`
  ...
  `;t.innerHTML=s,si(t),(n=t.querySelector(`#${i}`))==null||n.addEventListener("input",r=>{var l;const o=r.target.value.toLowerCase(),c=((l=t.querySelector("#pa-risk-filter"))==null?void 0:l.value)||"all";bt(t,o,c)}),(a=t.querySelector("#pa-risk-filter"))==null||a.addEventListener("change",r=>{var c;const o=((c=t.querySelector(`#${i}`))==null?void 0:c.value.toLowerCase())||"";bt(t,o,r.target.value)})}function bt(e,t,i){const s=e.querySelector("#pa-acct-tbody");s&&(s.innerHTML=nt.filter(n=>{const a=!t||n.upn.toLowerCase().includes(t)||n.name.toLowerCase().includes(t),r=i==="all"||n.risk===i;return a&&r}).map(n=>ii(n)).join(""),si(e))}function ii(e){return`
    <tr class="pa-acct-row" data-id="${e.id}">
      ...
      <td><div class="pill-group">${e.roles.map(bs).join("")}</div></td>
      <td><div class="pill-group">${fs(e.mfa)}</div></td>
      <td>${ft(e.risk)}</td>
  ...
  `,wt(t),t.querySelector("#pa-grp-search").addEventListener("input",i=>{const s=i.target.value.toLowerCase();t.querySelector("#pa-grp-tbody").innerHTML=Qe.filter(n=>!s||n.name.toLowerCase().includes(s)).map(n=>xt(n)).join(""),wt(t)})}function xt(e){return`
  ...
  `}function K(e,t,i){const s={add:"ti-user-plus",remove:"ti-user-minus",risk:"ti-alert-triangle",mfa:"ti-shield",tag:"ti-tag",review:"ti-clipboard-check"},n={add:"var(--clr-info-text)",remove:"var(--clr-warning-text)",risk:"var(--clr-danger-text)",mfa:"var(--clr-success-text)",tag:"var(--clr-info-text)",review:"var(--clr-success-text)"},a={add:"var(--clr-info-bg)",remove:"var(--clr-warning-bg)",risk:"var(--clr-danger-bg)",mfa:"var(--clr-success-bg)",tag:"var(--clr-info-bg)",review:"var(--clr-success-bg)"};ti.unshift({id:Date.now(),type:e,icls:s[e]||"ti-info-circle",ic:n[e]||"var(--clr-info-text)",bg:a[e]||"var(--clr-info-bg)",title:t,detail:t,by:i,time:"Just now"});const r=document.querySelector("#pa-tab-log");r&&ai(r)}function Ss(){const e=document.getElementById("page-licenses");if(!e)return;const t=[{name:"Microsoft 365 E3",total:600,consumed:581,available:19,status:"monitor",statusCls:"warning"},{name:"Microsoft 365 E5",total:150,consumed:148,available:2,status:"critical",statusCls:"danger"},{name:"Exchange Online P1",total:100,consumed:72,available:28,status:"healthy",statusCls:"success"},{name:"Power BI Pro",total:100,consumed:38,available:62,status:"healthy",statusCls:"success"}];e.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `,Ms(e)}function Ps(e){const t=e.status==="pending"||e.status==="overdue";return`
    <tr data-id="${e.id}">
      <td class="monospace">${e.id}</td>
      <td>${e.type}</td>
      <td>${e.requestor}</td>
  ...
  `;const n=e.querySelector("#portal-service-grid");Ie.forEach(a=>{var l,u;const r=De(kt(a.id)),o=fe(a.id)?se.reduce((y,re)=>{var _,v;return y+(((v=(_=ae(re.id))==null?void 0:_.operations)==null?void 0:v.length)||0)},0):((u=(l=ae(a.id))==null?void 0:l.operations)==null?void 0:u.length)||0,c=document.createElement("div");c.className=`portal-svc-card ${r?"":"disabled"}`,c.innerHTML=`
  ...
  `,s.querySelector("#svc-start-form").addEventListener("click",()=>{k="form",Ee={};const o=document.getElementById("page-portal");T(o)})}function Fs(e){var a;const t=rt(M),i=Ge(),s=ri(i,E);if(!s||!t){k="service",T(e);return}const n=be(s);e.innerHTML=`
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:10px">
  ...
  `,e.querySelector("#submit-new").addEventListener("click",()=>{k="landing",M=null,D=null,E=null,Ee={},T(e)}),e.querySelector("#submit-myreqs").addEventListener("click",()=>{Ds(()=>Promise.resolve().then(()=>Qa),void 0).then(c=>c.go("myreqs"))})}function js(){const e=document.getElementById("page-myreqs");e&&(e.innerHTML=`
    <div class="page-header">
...
Try rephrasing your question or ask about a specific service name.`}function Ks(){var t,i,s,n,a,r;const e=document.getElementById("page-graphapi");if(e){if(((t=d.currentUser)==null?void 0:t.role)!=="super"){e.innerHTML=`
  ...
  `;const r=a.querySelector("input");return r.addEventListener("change",()=>n(r.checked)),a}function Xs(){const e=document.getElementById("page-settings");e&&oi(e)}function oi(e){const t=d.settings;e.innerHTML=`
    <div class="page-header">
  ...
  `;const i=R({id:"toggle-ps",checked:t.showPSCommands,label:"Show PowerShell validation commands",sublabel:"Displays the PowerShell command used for each control in the M365 Config topic view.",onChange:v=>{d.settings.showPSCommands=v,m(),Ue()}});e.querySelector("#settings-ps-wrap").appendChild(i);const s=R({id:"toggle-result",checked:t.showTenantResult,label:"Show simulated tenant result",sublabel:"Displays the simulated tenant scan result for each control.",onChange:v=>{d.settings.showTenantResult=v,m(),Ue()}});e.querySelector("#settings-result-wrap").appendChild(s);const n=R({id:"toggle-expand",checked:t.autoExpandFailed,label:"Auto-expand failed controls",sublabel:"Automatically opens the details panel for failed controls on topic load.",onChange:v=>{d.settings.autoExpandFailed=v,m()}});e.querySelector("#settings-expand-wrap").appendChild(n);const a=R({id:"toggle-alert-fail",checked:t.agentAlertOnFail,label:"Alert on new failures",sublabel:"Send email notification when agent detects new failed controls.",onChange:v=>{d.settings.agentAlertOnFail=v,m()}});e.querySelector("#settings-alert-fail-wrap").appendChild(a);const r=R({id:"toggle-graph-health",checked:t.showGraphHealth,label:"Show Graph API health on dashboard",onChange:v=>{d.settings.showGraphHealth=v,m()}});e.querySelector("#settings-graph-health-wrap").appendChild(r);const o=R({id:"toggle-zt-score",checked:t.showZeroTrustScore,label:"Show Zero Trust score on dashboard",onChange:v=>{d.settings.showZeroTrustScore=v,m()}});e.querySelector("#settings-zt-score-wrap").appendChild(o);const c=R({id:"toggle-cfg-score",checked:t.showM365ConfigScore,label:"Show M365 Config score on dashboard",onChange:v=>{d.settings.showM365ConfigScore=v,m()}});e.querySelector("#settings-cfg-score-wrap").appendChild(c);const l=R({id:"toggle-portal-master",checked:t.portalEnabled!==!1,label:"Self-Service Portal — Master Switch",sublabel:"Disable to prevent all users from accessing the portal globally.",onChange:v=>{d.settings.portalEnabled=v,m();const w=e.querySelector("#portal-services-section");w&&(w.style.opacity=v?"1":"0.4"),p(v?"Self-Service Portal enabled.":"Self-Service Portal disabled globally.",v?"success":"warning")}});e.querySelector("#settings-portal-master-wrap").appendChild(l);const u=e.querySelector("#portal-services-section");u&&(u.style.opacity=t.portalEnabled!==!1?"1":"0.4");const y=e.querySelector("#portal-main-toggles");Ie.forEach(v=>{const w="portal_"+v.id.replace(/-/g,"_"),q=document.createElement("div");q.className="portal-svc-setting-row",q.innerHTML=`
    ...
    `,y.appendChild(q);const Ne=R({id:`chk-${w}`,checked:t[w]!==!1,label:"",onChange:Oe=>{d.settings[w]=Oe,m()}});q.querySelector(`#portal-toggle-${w}`).appendChild(Ne)});const re=e.querySelector("#portal-exchange-toggles"),_={"exchange-groups":"portal_exchange_groups","shared-mailbox":"portal_shared_mailbox","room-equipment":"portal_room_equipment","email-services":"portal_email_services"};se.forEach(v=>{const w=_[v.id],q=document.createElement("div");q.className="portal-svc-setting-row",q.innerHTML=`
      ...
      <span style="flex:1;font-size:11px;font-weight:500">${v.name}</span>
      <div id="portal-toggle-${w}"></div>
    `,re.appendChild(q);const Ne=R({id:`chk-${w}`,checked:t[w]!==!1,label:"",onChange:Oe=>{d.settings[w]=Oe,m()}});q.querySelector(`#portal-toggle-${w}`).appendChild(Ne)}),e.querySelector("#settings-save").addEventListener("click",()=>{d.settings.agentSchedule=e.querySelector("#settings-schedule").value,d.settings.agentAlertEmail=e.querySelector("#settings-alert-email").value,m(),p("Settings saved successfully.","success")}),e.querySelector("#settings-reset").addEventListener("click",()=>{gi(),oi(e),p("Settings reset to defaults.","info"),Ue()})}function xe(){return d.mcMessages||(d.mcMessages=S.map(e=>({id:e.id,status:e.status,tasksCreated:!1}))),d.mcMessages}function O(e){var t;return((t=xe().find(i=>i.id===e))==null?void 0:t.status)||"new"}function Rt(e,t){const i=xe().find(s=>s.id===e);i?i.status=t:xe().push({id:e,status:t,tasksCreated:!1}),m()}function li(e){var t;return((t=xe().find(i=>i.id===e))==null?void 0:t.tasksCreated)||!1}function ci(e){const t=xe().find(i=>i.id===e);t&&(t.tasksCreated=!0),m()}let B="all",g={search:"",service:"all",category:"all",actionRequired:"all",status:"all"},Z=new Set,V=!1;const Zs=[{id:"all",label:"All Messages",icon:"ti-inbox"},{id:"health",label:"Service Health",icon:"ti-heartbeat"},{id:"required",label:"Action Required",icon:"ti-alert-circle",badgeKey:"actionRequired"},{id:"upcoming",label:"Upcoming Changes",icon:"ti-calendar-event"},{id:"copilot",label:"Copilot",icon:"ti-sparkles"},{id:"licenses",label:"License Alerts",icon:"ti-license"},{id:"security",label:"Security",icon:"ti-shield-exclamation"}],ea=["All Services","Exchange Online","Microsoft Teams","SharePoint Online","OneDrive","Microsoft Entra ID","Microsoft Intune","Microsoft Copilot","Power Platform","Microsoft Defender","Microsoft 365"],ta=[{val:"all",label:"All Categories"},{val:"planForChange",label:"Plan for Change"},{val:"preventOrFixIssue",label:"Prevent or Fix Issue"},{val:"stayInformed",label:"Stay Informed"}];function ia(){const e=document.getElementById("page-msgcenter");e&&(Z=new Set,V=!1,pe(e))}function pe(e){var n;const t=S.filter(a=>a.actionRequired&&O(a.id)!=="actioned").length,i=S.filter(a=>O(a.id)==="new").length,s=F.filter(a=>a.status!=="resolved").length;e.innerHTML=`
    <div class="page-header">
      <div>
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-tile">
        <div class="kpi-value success">${S.filter(a=>O(a.id)==="actioned").length}</div>
    ...
    <!-- Digest overlay -->
    ${V?la():""}
  `,e.querySelectorAll("#mc-tabs .tab-btn").forEach(a=>{a.addEventListener("click",()=>{B=a.dataset.tab,Z=new Set,pe(e)})}),e.querySelector("#mc-sync").addEventListener("click",()=>ca(e)),e.querySelector("#mc-digest").addEventListener("click",()=>{V=!V,pe(e)}),e.querySelector("#mc-create-tasks").addEventListener("click",()=>{S.filter(r=>r.actionRequired&&O(r.id)!=="actioned").forEach(r=>{li(r.id)||ci(r.id)}),p(`${t} task groups created from action-required messages.`,"success")}),B==="health"?oa(e):z(e),V&&((n=e.querySelector("#mc-digest-close"))==null||n.addEventListener("click",()=>{V=!1,pe(e)}))}function sa(){let e=[...S];if(B==="required"&&(e=e.filter(t=>t.actionRequired&&O(t.id)!=="actioned")),B==="upcoming"&&(e=e.filter(t=>t.targetRelease)),B==="copilot"&&(e=e.filter(t=>t.service==="Microsoft Copilot"||t.tags.some(i=>i.toLowerCase().includes("copilot")||i.toLowerCase().includes("ai")))),B==="licenses"&&(e=e.filter(t=>t.tags.some(i=>i.toLowerCase().includes("licens")))),B==="security"&&(e=e.filter(t=>t.tags.some(i=>["security","mfa","authentication","defender","conditional access","zero trust"].includes(i.toLowerCase())))),g.search){const t=g.search.toLowerCase();e=e.filter(i=>i.id.toLowerCase().includes(t)||i.title.toLowerCase().includes(t)||i.aiSummary.toLowerCase().includes(t))}return g.service!=="all"&&(e=e.filter(t=>t.service===g.service)),g.category!=="all"&&(e=e.filter(t=>t.category===g.category)),g.actionRequired==="yes"&&(e=e.filter(t=>t.actionRequired)),g.actionRequired==="no"&&(e=e.filter(t=>!t.actionRequired)),g.status!=="all"&&(e=e.filter(t=>O(t.id)===g.status)),e}function z(e){const t=e.querySelector("#mc-content");if(!t)return;const i=sa();t.innerHTML=`
    <!-- Filter bar -->
  ...
  `,t.querySelector("#mc-search").addEventListener("input",s=>{g.search=s.target.value,z(e)}),t.querySelector("#mc-svc-filter").addEventListener("change",s=>{g.service=s.target.value,z(e)}),t.querySelector("#mc-cat-filter").addEventListener("change",s=>{g.category=s.target.value,z(e)}),t.querySelector("#mc-req-filter").addEventListener("change",s=>{g.actionRequired=s.target.value,z(e)}),t.querySelector("#mc-status-filter").addEventListener("change",s=>{g.status=s.target.value,z(e)}),ra(t,e)}function aa(e){const t=C[e.service]||{icon:"ti-apps",color:"#185FA5",bg:"#E6F1FB"},i=O(e.id),s=Z.has(e.id),n=li(e.id),a={high:'<span class="badge danger">High</span>',medium:'<span class="badge warning">Medium</span>',low:'<span class="badge neutral">Low</span>'}[e.severity]||"",r={planForChange:'<span class="badge info">Plan for Change</span>',preventOrFixIssue:'<span class="badge danger">Prevent / Fix Issue</span>',stayInformed:'<span class="badge neutral">Stay Informed</span>'}[e.category]||"",o={new:'<span class="mc-status-dot new"></span>',read:"",actioned:'<span class="badge success" style="font-size:9px">✓ Actioned</span>',dismissed:'<span class="badge neutral" style="font-size:9px">Dismissed</span>'}[i]||"";return`
      ...
      <div class="digest-panel">
        ...
        <div class="digest-body">
          ...
          <div style="font-size:11px;color:var(--color-text-secondary);line-height:1.7">
            ${F.filter(i=>i.status!=="resolved").length===0?"✅ All Microsoft 365 services are operating normally.":F.filter(i=>i.status!=="resolved").map(i=>`⚠️ <strong>${i.service}</strong>: ${i.title}`).join("<br>")}
            <br>✅ ${F.filter(i=>i.status==="resolved").length} incidents resolved since last digest.
          </div>
...
→ **Applications → Secret & Certificate Expiry** for rotation roadmap.`}];let Ae="executive",A={type:"all",status:"all",search:""},ve=[],qt=!1,Y=[],Q=[];const pa=[{id:"executive",label:"Executive",icon:"ti-layout-dashboard"},{id:"appregistrations",label:"App Registrations",icon:"ti-app-window"},{id:"enterprise",label:"Enterprise Apps",icon:"ti-grid-dots"},{id:"secrets",label:"Secrets & Certs",icon:"ti-lock"},{id:"permissions",label:"Permissions",icon:"ti-shield-check"},{id:"consents",label:"Admin Consents",icon:"ti-user-check"},{id:"owners",label:"Owners",icon:"ti-users"},{id:"usage",label:"Usage Analytics",icon:"ti-chart-line"},{id:"risk",label:"Risk Assessment",icon:"ti-alert-triangle"},{id:"lifecycle",label:"Lifecycle",icon:"ti-timeline"},{id:"recommendations",label:"Recommendations",icon:"ti-checklist"},{id:"copilot",label:"App Copilot",icon:"ti-robot"}];async function ua(){const e=document.getElementById("page-applications");if(e){e.innerHTML='<div style="padding:20px;text-align:center"><div class="spinner"></div><p>Loading real M365 application data...</p></div>';try{console.log("📡 Fetching real application data from backend...");const t=await $i(),i=await Pi();t.success?(Y=t.data||U,console.log(`✅ Loaded ${Y.length} real applications from API`)):(console.warn("⚠️ Failed to fetch applications, using simulated data:",t.error),Y=U),i.success?(Q=i.data||$e,console.log(`✅ Loaded ${Q.length} real service principals from API`)):(console.warn("⚠️ Failed to fetch service principals, using simulated data"),Q=$e)}catch(t){console.error("❌ Error loading application data:",t),Y=U,Q=$e}H(e)}}function H(e){var n,a;const t=W.filter(r=>r.status==="expiring").length,i=W.filter(r=>r.status==="expired").length,s=we.filter(r=>r.severity==="critical").length;e.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `,e.querySelectorAll(".app-tab-btn").forEach(r=>{r.addEventListener("click",()=>{Ae=r.dataset.appSection,H(e)})}),(n=e.querySelector("#app-refresh"))==null||n.addEventListener("click",()=>{const r=e.querySelector("#app-refresh");r.innerHTML='<span class="spinner dark"></span> Scanning...',r.disabled=!0,setTimeout(()=>{r.innerHTML='<i class="ti ti-refresh"></i> Refresh',r.disabled=!1,p("Application inventory updated — 87 app registrations, 124 service principals scanned.","success")},2200)}),(a=e.querySelector("#app-audit"))==null||a.addEventListener("click",()=>p("Application audit exported as CSV.","success")),Ea(e)}function va(){return({executive:Lt,appregistrations:ga,enterprise:ma,secrets:ya,permissions:ha,consents:fa,owners:ba,usage:xa,risk:wa,lifecycle:Aa,recommendations:Sa,copilot:ka}[Ae]||Lt)()}function Lt(){const e=le,t=W.filter(a=>a.status==="expiring").length,i=W.filter(a=>a.status==="expired").length,s=we.filter(a=>a.severity==="critical").length,n=ue.filter(a=>a.status==="unused").length;return`
    <div class="grid-2 mb-3" style="gap:16px">
      <div class="card">
        <div class="card-header">
  ...
  `}function ga(){const e=Y.length>0?Y:U,t=e.filter(s=>!(A.type!=="all"&&s.category!==A.type||A.status!=="all"&&s.status!==A.status||A.search&&!s.name.toLowerCase().includes(A.search.toLowerCase()))),i=[...new Set(e.map(s=>s.category))];return`
    <div class="filter-bar mb-3">
  ...
  `}function ya(){const e=W.filter(s=>s.status==="expired"),t=W.filter(s=>s.status==="expiring"),i=W.filter(s=>s.status==="healthy");return`
    ${e.length>0?`
      <div class="alert-banner danger mb-3">
        <i class="ti ti-alert-triangle"></i>
  ...
  `}function fa(){const e=Tt.filter(i=>i.scope==="Tenant-wide"),t=Tt.filter(i=>i.scope==="User");return`
    ...
    `).join("")}
  `}function ba(){const e=U.filter(s=>s.owners.length===0),t=U.filter(s=>s.owners.length===1),i=U.filter(s=>s.owners.length>1);return`
    ${e.length>0?`
      <div class="alert-banner danger mb-3">
        <i class="ti ti-alert-triangle"></i>
  ...
  `}function wa(){const e=we.filter(i=>i.severity==="critical"),t=we.filter(i=>i.severity==="high");return`
    ...
    `).join("")}
  `}function Aa(){const e=U.filter(s=>{const n=new Date(s.created),a=new Date(Date.now()-30*24*60*60*1e3);return n>a}),t=U.filter(s=>s.owners.length===0||s.status==="inactive"&&s.risk==="critical"),i=ue.filter(s=>s.status==="unused");return`
...
Current state: **87 app registrations**, **124 enterprise apps**, **5 expiring secrets (30d)**, **2 critical risk apps**`}],qt=!0);const e=["Show expiring secrets","Which apps have Directory.ReadWrite.All?","List apps without owners","Show high-risk applications","Unused apps (90+ days)","Multi-tenant applications"];return`
  ...
  `}function Gt(e){return`<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:4px">
    ...
    `).join("")}
  </div>`}function Ea(e){var n,a,r,o,c;const t=e.querySelector("#app-content");if(!t)return;(n=t.querySelector("#app-search"))==null||n.addEventListener("input",l=>{A.search=l.target.value,H(e)}),(a=t.querySelector("#app-type-filter"))==null||a.addEventListener("change",l=>{A.type=l.target.value,H(e)}),(r=t.querySelector("#app-status-filter"))==null||r.addEventListener("change",l=>{A.status=l.target.value,H(e)}),(o=t.querySelector("#exec-view-risk"))==null||o.addEventListener("click",()=>{Ae="risk",H(e)}),(c=t.querySelector("#exec-view-recs"))==null||c.addEventListener("click",()=>{Ae="recommendations",H(e)});const i=t.querySelector("#app-cop-send"),s=t.querySelector("#app-cop-input");i&&s&&(i.addEventListener("click",()=>ze(e,s)),s.addEventListener("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),ze(e,s))})),t.querySelectorAll(".app-cop-pill").forEach(l=>{l.addEventListener("click",()=>{const u=t.querySelector("#app-cop-input");u&&(u.value=l.dataset.q,ze(e,u))})})}function ze(e,t){const i=t.value.trim();if(!i)return;ve.push({role:"user",text:i}),t.value="";const s=e.querySelector("#app-cop-msgs");s&&(s.innerHTML+=`<div class="chat-msg user-msg" style="max-width:85%;margin-bottom:12px">
      <div class="chat-sender" style="justify-content:flex-end">You</div>
      <div class="chat-bubble">${i}</div>
  ...
  `}function Oa(){const e=$a;return`
    <div class="grid-2 mb-3" style="gap:16px">
      <div class="card">
        ...
        ${ui([{label:"Defender Enabled",val:e.antivirus.defenderEnabled,cls:"success"},{label:"Real-Time Protection",val:e.antivirus.realTimeProtection,cls:"success"},{label:"Cloud Protection",val:e.antivirus.cloudProtection,cls:"success"},{label:"Firewall Enabled",val:e.firewall.enabled,cls:"success"}])}
      </div>
...
Based on your question, navigate to the relevant section above. Current status: 847 devices, 98.2% compliant, 2 critical risks.`;qe.push({role:"ai",text:r}),s&&(s.innerHTML+=`<div class="chat-msg ai" style="max-width:85%;margin-bottom:12px">
  ...
  `,Ai(),Si(),((i=d.currentUser)==null?void 0:i.role)==="manager"){const s=document.getElementById("alert-approvals-link");s&&s.addEventListener("click",async n=>{n.preventDefault(),await x("approvals")})}}function Ya(){return["dashboard","requests","security","zerotrust","privaccts","m365config","msgcenter","applications","intune","licenses","agents","approvals","portal","myreqs","chat","graphapi","sso","audit","settings"].map(t=>`<div class="page" id="page-${t}"></div>`).join("")}Va();Bt().catch(e=>{console.error("Login render error:",e),Bt()});const Qa=Object.freeze(Object.defineProperty({__proto__:null,go:x,hasAccess:mi,isRole:Ja,resetSettings:gi,saveState:m,state:d},Symbol.toStringTag,{value:"Module"}));
