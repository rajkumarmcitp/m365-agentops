    ...
    <div class="grid-2" style="gap:16px">
      ...
      <div>
        ...
        <div class="card mb-3">
          <div class="card-header"><span class="card-title">Approval Workflow</span></div>
          <div style="padding:16px;border-top:0.5px solid var(--color-border-secondary)">
            ${(((o=t.validation)==null?void 0:o.approvalPath)||[]).map((d,c)=>{const p=t.approvals.find(g=>g.step===d),y=!p&&d!=="agent"&&d!=="action",b=(p==null?void 0:p.status)==="APPROVED";return`
                <div style="display:flex;gap:10px;margin-bottom:12px;align-items:flex-start">
...
Current status: Secure Score 64/95 · ${R.filter(l=>l.status!=="resolved").length} active incidents · ${P.highRiskUsers} high-risk users.`;Re.push({role:"ai",text:r}),s&&(s.innerHTML+=`<div class="chat-msg ai" style="max-width:85%;margin-bottom:12px">
  ...
  `;const a=e.querySelector("#portal-service-grid");Qe.forEach(n=>{var d,c;const r=Xe(pi(n.id)),l=je(n.id)?ke.reduce((p,y)=>{var b,g;return p+(((g=(b=Se(y.id))==null?void 0:b.operations)==null?void 0:g.length)||0)},0):((c=(d=Se(n.id))==null?void 0:d.operations)==null?void 0:c.length)||0,o=document.createElement("div");o.className=`portal-svc-card ${r?"":"disabled"}`,o.innerHTML=`
...
Try rephrasing your question or ask about a specific service name.`}function mr(){var t,i,s,a,n,r;const e=document.getElementById("page-graphapi");if(e){if(((t=f.currentUser)==null?void 0:t.role)!=="super"){e.innerHTML=`
  ...
  `;const i=Y({id:"toggle-ps",checked:t.showPSCommands,label:"Show PowerShell validation commands",sublabel:"Displays the PowerShell command used for each control in the M365 Config topic view.",onChange:g=>{f.settings.showPSCommands=g,N()}});e.querySelector("#settings-ps-wrap").appendChild(i);const s=Y({id:"toggle-result",checked:t.showTenantResult,label:"Show simulated tenant result",sublabel:"Displays the simulated tenant scan result for each control.",onChange:g=>{f.settings.showTenantResult=g,N()}});e.querySelector("#settings-result-wrap").appendChild(s);const a=Y({id:"toggle-expand",checked:t.autoExpandFailed,label:"Auto-expand failed controls",sublabel:"Automatically opens the details panel for failed controls on topic load.",onChange:g=>{f.settings.autoExpandFailed=g,N()}});e.querySelector("#settings-expand-wrap").appendChild(a);const n=Y({id:"toggle-alert-fail",checked:t.agentAlertOnFail,label:"Alert on new failures",sublabel:"Send email notification when agent detects new failed controls.",onChange:g=>{f.settings.agentAlertOnFail=g,N()}});e.querySelector("#settings-alert-fail-wrap").appendChild(n);const r=Y({id:"toggle-graph-health",checked:t.showGraphHealth,label:"Show Graph API health on dashboard",onChange:g=>{f.settings.showGraphHealth=g,N()}});e.querySelector("#settings-graph-health-wrap").appendChild(r);const l=Y({id:"toggle-zt-score",checked:t.showZeroTrustScore,label:"Show Zero Trust score on dashboard",onChange:g=>{f.settings.showZeroTrustScore=g,N()}});e.querySelector("#settings-zt-score-wrap").appendChild(l);const o=Y({id:"toggle-cfg-score",checked:t.showM365ConfigScore,label:"Show M365 Config score on dashboard",onChange:g=>{f.settings.showM365ConfigScore=g,N()}});e.querySelector("#settings-cfg-score-wrap").appendChild(o);const d=Y({id:"toggle-portal-master",checked:t.portalEnabled!==!1,label:"Self-Service Portal — Master Switch",sublabel:"Disable to prevent all users from accessing the portal globally.",onChange:g=>{f.settings.portalEnabled=g,N();const m=e.querySelector("#portal-services-section");m&&(m.style.opacity=g?"1":"0.4"),u(g?"Self-Service Portal enabled.":"Self-Service Portal disabled globally.",g?"success":"warning")}});e.querySelector("#settings-portal-master-wrap").appendChild(d);const c=e.querySelector("#portal-services-section");c&&(c.style.opacity=t.portalEnabled!==!1?"1":"0.4");const p=e.querySelector("#portal-main-toggles");Qe.forEach(g=>{const m="portal_"+g.id.replace(/-/g,"_"),v=document.createElement("div");v.className="portal-svc-setting-row",v.innerHTML=`
  ...
  `;try{const a=await Ii(),n=await zi();if(!a.success||!a.data||a.data.length===0){fi(e);return}const r=new Date,l=new Date(r.getTime()-7*24*60*60*1e3),o=messages.filter(p=>new Date(p.publishedDate)>=l&&p.actionRequired),d=o.length,c=o.filter(p=>p.severity==="high").length;e.innerHTML=`
      <div class="page-header">
        <div>
          <div class="page-title"><i class="ti ti-antenna"></i> Change Intelligence</div>
          <div class="page-subtitle">Last 7 days · Action required only · ${o.length} announcements · <span style="color:var(--clr-success-text)">● Real data</span></div>
        </div>
        <div class="page-actions">
          <button class="btn" id="mc-sync"><i class="ti ti-refresh"></i> Sync now</button>
          <button class="btn" id="mc-analyze"><i class="ti ti-sparkles"></i> Analyze with AI</button>
          <button class="btn btn-primary" id="mc-create-tasks"><i class="ti ti-circle-plus"></i> Create tasks (${d})</button>
        </div>
      </div>
      ...
      <div class="kpi-row">
        ...
        <div class="kpi-tile">
          <div class="kpi-value danger">${d}</div>
          <div class="kpi-label">Action Required</div>
        </div>
        <div class="kpi-tile">
          <div class="kpi-value warning">${c}</div>
          <div class="kpi-label">High Severity</div>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <span class="badge danger">${o.length} announcements</span>
        </div>
          ...
          ${o.slice(0,50).map(p=>`
            <div style="padding:12px;border-bottom:0.5px solid var(--color-border-tertiary)">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:6px">
                <div style="flex:1">
                  <div style="font-weight:600;font-size:12px;color:var(--color-text-primary)">${p.title||p.id}</div>
                  <div style="font-size:10px;color:var(--color-text-secondary);margin-top:4px">
                    Service: <strong>${p.service||"Unknown"}</strong> · ID: <code style="font-family:monospace;font-size:9px">${p.id}</code>
                  </div>
                </div>
                <span class="badge ${p.severity==="high"?"danger":p.severity==="medium"?"warning":"success"}" style="flex-shrink:0;font-size:9px">
                  ${p.severity||"normal"}
                </span>
              </div>
              ${p.actionByDate?`<div style="font-size:10px;color:var(--clr-danger-text);margin-top:6px">⚠️ Action required by: <strong>${p.actionByDate}</strong></div>`:""}
              ${p.body&&typeof p.body=="string"?`<div style="font-size:10px;color:var(--color-text-secondary);margin-top:6px;line-height:1.4">${p.body.substring(0,150)}...</div>`:""}
            </div>
          `).join("")}
        </div>
      </div>
    `,(t=e.querySelector("#mc-sync"))==null||t.addEventListener("click",async()=>{const p=e.querySelector("#mc-sync");p.innerHTML='<span class="spinner dark"></span> Syncing...',p.disabled=!0,setTimeout(async()=>{p.innerHTML='<i class="ti ti-refresh"></i> Sync now',p.disabled=!1,await ps(e),u(`Synced ${messages.length} messages`,"success")},2e3)}),(i=e.querySelector("#mc-analyze"))==null||i.addEventListener("click",()=>{if(d===0){u("No recent action-required announcements to analyze","info");return}u(`Analyzing ${d} announcements with AI...`,"info"),setTimeout(()=>{u("AI analysis complete - see insights panel below","success")},2e3)}),(s=e.querySelector("#mc-create-tasks"))==null||s.addEventListener("click",()=>{d===0?u("No action-required announcements to create tasks for","info"):u(`Creating ${d} tasks from recent announcements...`,"success")})}catch(a){console.error("Error loading Message Center:",a),fi(e)}}function fi(e){e.innerHTML=`
    <div class="page-header">
      <div>
        <div class="page-title"><i class="ti ti-antenna"></i> Change Intelligence</div>
...
**Current state:** ${t.totalManagedDevices||0} managed devices, ${t.compliancePercentage||0}% compliant, ${t.deviceHealthScore||0}/100 health score, ${i.criticalRiskCount||0} critical risks`}],ki=!0}const e=["Show device health summary","Patch management status","Encryption coverage","Firewall & protection status","Critical risk devices","Compliance policies"];return`
  ...
  `}function ys(e){return`<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:4px">
    ...
    `).join("")}
      ...
      <div class="chat-sender" style="justify-content:flex-end">You</div>
      <div class="chat-bubble">${i}</div>
    </div>`,s.scrollTop=s.scrollHeight),setTimeout(()=>{var c,p,y,b,g;const a=i.toLowerCase(),n=x.summary||{},r=x.riskAssessment||{},l=x.patchManagement||{},o=x.endpointSecurity||{};let d=`Based on your question about Intune, here's the current tenant status:
...
Ask me about device health, compliance, security, patches, or recommendations!`;tt.push({role:"ai",text:d}),s&&(s.innerHTML+=`<div class="chat-msg ai" style="max-width:85%;margin-bottom:12px">
  ...
  `,e.querySelectorAll(".intune-tab-btn").forEach(i=>{i.addEventListener("click",()=>{De=i.dataset.tab,e.querySelector("#myacc-content").innerHTML=Ei(),De==="signin"&&setTimeout(()=>Pi(e),100)})}),De==="signin"&&setTimeout(()=>Pi(e),100),(t=e.querySelector("#myacc-refresh"))==null||t.addEventListener("click",()=>{const i=e.querySelector("#myacc-refresh");i.innerHTML='<span class="spinner dark"></span> Refreshing...',i.disabled=!0,setTimeout(()=>{i.innerHTML='<i class="ti ti-refresh"></i> Refresh',i.disabled=!1,u("Your profile has been refreshed.","success")},1500)})}function Ei(){const t={executive:ho,profile:bo,security:xo,signin:wo,licenses:Ao,groups:ko,devices:So,apps:$o,onedrive:Eo,teams:Po,approvals:Co,copilot:Mo}[De];return t?t():""}function ho(){var c,p,y,b,g,m,v,A;const e=((c=h.security)==null?void 0:c.securityScore)||85,t=((p=h.security)==null?void 0:p.mfaStatus)||"Not Enabled",i=((y=h.security)==null?void 0:y.riskLevel)||"Low",s=(h.licenses&&Array.isArray(h.licenses)?h.licenses.length:0)||((b=h.licenses)==null?void 0:b.count)||0,a=(h.devices&&Array.isArray(h.devices)?h.devices.length:0)||((g=h.devices)==null?void 0:g.count)||0,n=(h.groups&&h.groups.securityGroups?h.groups.securityGroups.length:0)||0,r=((m=h.teams)==null?void 0:m.teamsMembership)||0,l=0,o=((v=h.onedrive)==null?void 0:v.percentageUsed)||0,d=((A=h.mailbox)==null?void 0:A.mailboxUsage)||65;return`
    <div class="kpi-row mb-3">
      <div class="kpi-tile">
