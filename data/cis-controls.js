    ...
    subsections: [
      {
        ...
        name: '7.2 Policies',
        controls: [
          { id: '7.2.1', title: 'Ensure modern authentication for SharePoint applications is required', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Modern auth is required for MFA to work with SharePoint Online clients. Requires manual verification in SharePoint admin center.', ps: null },
          { id: '7.2.2', title: 'Ensure SharePoint and OneDrive integration with Azure AD B2B is enabled', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'B2B integration ensures external sharing uses Entra ID guest accounts for governance.', ps: null },
          { id: '7.2.3', title: 'Ensure external content sharing is restricted', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Restricting external sharing prevents unauthorized access by external parties.', ps: null },
          { id: '7.2.4', title: 'Ensure OneDrive content sharing is restricted', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'OneDrive sharing restrictions prevent personal file shares from leaking corporate data.', ps: null },
          { id: '7.2.5', title: "Ensure that SharePoint guest users cannot share items they don't own", type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Preventing guest re-sharing limits uncontrolled propagation of shared content.', ps: null },
          { id: '7.2.6', title: 'Ensure SharePoint external sharing is restricted', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Restricting external sharing prevents widespread external access.', ps: null },
          { id: '7.2.7', title: 'Ensure link sharing is restricted in SharePoint and OneDrive', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Restricting link sharing prevents anonymous external access.', ps: null },
          { id: '7.2.8', title: 'Ensure external sharing is restricted by security group', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Restricting sharing via security groups ensures controlled external access.', ps: null },
          { id: '7.2.9', title: 'Ensure guest access to a site or OneDrive will expire automatically', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Automatic guest expiration removes stale external access over time.', ps: null },
          { id: '7.2.10', title: 'Ensure reauthentication with verification code is restricted', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Verification code restrictions reduce risk from phishing and compromised credentials.', ps: null },
          { id: '7.2.11', title: 'Ensure the SharePoint default sharing link permission is set', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Setting default permissions ensures appropriate sharing scope by default.', ps: null },
        ],
      },
      {
        id: 't7s2',
        name: '7.3 Settings',
        controls: [
          { id: '7.3.1', title: 'Ensure Office 365 SharePoint infected files are disallowed for download', type: 'manual', profile: 'E3 L1', status: 'pass', value: null, desc: 'Blocking download of infected files prevents malware distribution to endpoints.', ps: null },
        ],
      },
    ],
  },
  ...
  // ============================================================
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        name: '8.5 Meetings',
        controls: [
          { id: '8.5.1', title: "Ensure anonymous users can't join a meeting", type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Anonymous join: Disabled.', desc: 'Blocking anonymous users prevents uninvited parties from attending meetings.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AllowAnonymousUsersToJoinMeeting` },
          { id: '8.5.2', title: "Ensure anonymous users and dial-in callers can't start a meeting", type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Anonymous/dial-in start: Disabled.', desc: 'Preventing anonymous/dial-in start ensures only approved users initiate meetings.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AllowAnonymousUsersToStartMeeting,AllowPstnDialInUnavailableTranscribing` },
          { id: '8.5.3', title: 'Ensure only people in my org can bypass the lobby', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Lobby bypass: Org users only.', desc: 'The meeting lobby provides essential control over participant access.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AutoAdmitUsers` },
          { id: '8.5.4', title: "Ensure users dialing in can't bypass the lobby", type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Dial-in bypass: Disabled.', desc: 'Dial-in users must wait in lobby for organizer approval.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AllowPstnDialInUnavailableTranscribing` },
          { id: '8.5.5', title: 'Ensure meeting chat does not allow anonymous users', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Anonymous chat: Disabled.', desc: 'Blocking anonymous chat prevents uninvited contributions to meeting discussions.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select MeetingChatEnabledType` },
          { id: '8.5.6', title: 'Ensure only organizers and co-organizers can present', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Presenter role: Organizer/co-organizer only.', desc: 'Restricting presenters limits meeting control to authorized users.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AllowUserToBePresenter` },
          ...
          { id: '8.5.8', title: 'Ensure external meeting chat is off', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'External chat: Disabled.', desc: 'Disabling external chat prevents unmonitored external communications during meetings.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select MeetingChatEnabledType` },
          { id: '8.5.9', title: 'Ensure meeting recording is off by default', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Recording default: Disabled.', desc: 'Default-off recording requires explicit opt-in for participant privacy.', ps: `Get-CsTeamsMeetingPolicy -Identity Global | Select AllowCloudRecording` },
        ],
      },
      {
        id: 't8s5',
        name: '8.6 Messaging',
        controls: [
          { id: '8.6.1', title: 'Ensure users can report security concerns in Teams', type: 'manual', profile: 'E3 L1', status: 'pass', value: 'Security reporting: Enabled.', desc: 'Enabling security reporting empowers users to flag suspicious content.', ps: `Get-CsTeamsClientConfiguration | Select AllowSecurityReportingOption` },
        ],
      },
    ],
  },
