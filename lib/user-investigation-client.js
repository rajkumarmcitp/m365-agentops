...
export async function getUserProfile(userId) {
  ...
  const url = `${API_BASE}/user-investigation/user-profile?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: {} }
    return await response.json()
...
export async function getUserGroups(userId) {
  ...
  const url = `${API_BASE}/user-investigation/user-groups?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: [] }
    return await response.json()
...
export async function getDirectoryRoles(userId) {
  const params = new URLSearchParams()
  params.append('userId', userId)

  const url = `${API_BASE}/user-investigation/directory-roles?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: [] }
    return await response.json()
  } catch (error) {
    console.error('Failed to fetch directory roles:', error)
    return { success: false, data: [] }
  }
}

export async function getAuthenticationMethods(userId) {
  const params = new URLSearchParams()
  params.append('userId', userId)

  const url = `${API_BASE}/user-investigation/authentication-methods?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: [] }
    return await response.json()
...
export async function getMailboxSettings(userId) {
  const params = new URLSearchParams()
  params.append('userId', userId)

  const url = `${API_BASE}/user-investigation/mailbox-settings?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: {} }
    return await response.json()
...
export async function getRiskyUser(userId) {
  const params = new URLSearchParams()
  params.append('userId', userId)

  const url = `${API_BASE}/user-investigation/risky-users?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: {} }
    return await response.json()
  } catch (error) {
    console.error('Failed to fetch risky user:', error)
    return { success: false, data: {} }
  }
}

export async function getEnterpriseApps(userId) {
  const params = new URLSearchParams()
  params.append('userId', userId)

  const url = `${API_BASE}/user-investigation/enterprise-apps?${params.toString()}`
  try {
    const response = await fetch(url)
    if (!response.ok) return { success: false, data: [] }
    return await response.json()
