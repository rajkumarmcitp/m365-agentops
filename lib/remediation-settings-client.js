...
export async function getRemediationSettings() {
  try {
    const response = await fetch('http://localhost:3001/api/tenantguard/settings/remediation')
...
export async function getAutoFixHistory() {
  try {
    const response = await fetch('http://localhost:3001/api/tenantguard/auto-fix/history')
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
    return data.data || []
  } catch (err) {
