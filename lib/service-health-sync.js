...
async function performSync() {
  ...
  try {
    ...
    const response = await fetch(
      `${getApiUrl()}/servicehealth/messages?siteId=${encodeURIComponent(syncState.siteId)}&listId=${encodeURIComponent(syncState.listId)}`
    )
    ...
    syncState.messages = result.messages.map(msg => ({
      id: msg.id,
    ...
    console.log(
      `[Service Health Sync] ✓ Successfully synced ${syncState.messages.length} messages in ${duration}ms (source: ${result.source})`
    )
