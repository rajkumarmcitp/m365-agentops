...
export async function getOpenDrifts() {
  try {
    const response = await fetch('http://localhost:3000/api/tenantguard/compliance/drifts')
...
export async function getDriftHistory(controlId) {
  try {
    const response = await fetch(`http://localhost:3000/api/tenantguard/compliance/drifts/${controlId}`)
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
    return data.data || []
  } catch (err) {
...
export async function getRecommendation(driftId) {
  try {
    const response = await fetch(`http://localhost:3000/api/tenantguard/compliance/recommendations/${driftId}`)
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
...
export async function approveRecommendation(recId, notes = '') {
  try {
    const response = await fetch(`http://localhost:3000/api/tenantguard/compliance/recommendations/${recId}/approve`, {
      method: 'POST',
...
export async function rejectRecommendation(recId, reason = '') {
  try {
    const response = await fetch(`http://localhost:3000/api/tenantguard/compliance/recommendations/${recId}/reject`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function markDriftResolved(driftId, notes = '') {
  try {
    const response = await fetch(`http://localhost:3000/api/tenantguard/compliance/drifts/${driftId}/resolve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ notes })
    })
...
export async function getComplianceStats() {
  try {
    const response = await fetch('http://localhost:3000/api/tenantguard/compliance/stats')
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
    return data.data || {}
  } catch (err) {
