...
export async function getCurrentRiskAssessment() {
  try {
    const response = await fetch('http://localhost:3001/api/tenantguard/risk/current')
...
export async function getRiskHistory(limit = 10) {
  try {
    const response = await fetch(`http://localhost:3001/api/tenantguard/risk/history?limit=${limit}`)
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
    return data.data || []
  } catch (err) {
...
export async function getRiskFactors() {
  try {
    const response = await fetch('http://localhost:3001/api/tenantguard/risk/factors')
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    const data = await response.json()
    return data.data || []
  } catch (err) {
...
export async function triggerRiskAssessment() {
  try {
    const response = await fetch('http://localhost:3001/api/tenantguard/risk/trigger', {
      method: 'POST',
