...
async function addServiceHealthColumns(siteId, listId) {
  const columns = [
    ...
    },
    {
      ...
      columnIndexed: true,
      choice: {
        choices: ['Advisory', 'Incident', 'Service Degradation', 'Maintenance', 'Investigation'],
        allowTextEntry: true,
        displayAs: 'dropDownMenu'
      }
    },
...
export async function getServiceHealthMessages(siteId, listId) {
  try {
    // Try to get token - if not available, return demo data
    let token
    try {
      token = await getAccessToken()
    } catch (tokenError) {
      console.warn('[Service Health] No auth token - using demo data for Phase 4')
    ...
    if (!response.ok) {
      console.warn(`[Service Health] Graph API error (${response.status}) - using demo data`)
      return getDemoMessages()
    }
    ...
    console.warn('[Service Health] Error fetching from SharePoint - using demo data:', error.message)
    return getDemoMessages()
  }
}

/**
 * Demo messages for Phase 4 testing (no SharePoint needed)
 * @returns {Array} Demo messages
 */
function getDemoMessages() {
  return [
    ...
    },
    {
      ...
      Description: 'Search results delayed in SharePoint sites',
