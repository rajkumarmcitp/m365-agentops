...
export async function generateInvestigationAnalysis(userData) {
  try {
    ...
    // Try to load Anthropic SDK - if it fails, use fallback
    if (!Anthropic) {
      try {
        ...
        console.warn('⚠️ Anthropic SDK unavailable:', importError.message);
        return {
          success: false,
    ...
    const analysis = await runInvestigationAgent(client, normalizedData);
    ...
    console.error('⚠️ Investigation analysis error:', error.message);
 ...
 */
async function runInvestigationAgent(client, data) {
