...
async function loadCAPData(el) {
  try {
    const [homeData, complianceData, controlsData, riskData, remData, driftData, userData, approvalsData] = await Promise.all([
      ...
      callAPI('/me'),
      callAPI(`/cap/controls/approvals/${currentCategory}`)
    ])

    // Store user info
    currentUser = userData?.data || userData || {}

    // Store manual validation approvals
    if (approvalsData?.data) {
      approvalsData.data.forEach(approval => {
        manualValidationApprovals[approval.controlId] = approval
      })
    }
  ...
  setTimeout(() => {
    ...
    categoryButtons.forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        // Load approvals for new category
        const approvalsData = await callAPI(`/cap/controls/approvals/${currentCategory}`)
        if (approvalsData?.data) {
...
function renderCAP(el) {
  el.innerHTML = `
    ...
    <div class="cap-tabs">
      ...
      <button class="tab-btn" data-tab="assessment">Zero Trust</button>
...
function renderCategoryAssessmentTab(el, data) {
        ...
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead>
            <tr style="background:var(--color-background-secondary);border-bottom:1px solid var(--color-border-tertiary)">
              ...
              <th style="padding:10px;text-align:left;font-weight:600;color:var(--color-text-primary);width:120px">Validation</th>
          ...
          <tbody>
            ${categoryEval.controls.map((control, i) => `
              <tr style="border-bottom:0.5px solid var(--color-border-tertiary);${i % 2 === 0 ? 'background:var(--color-background-primary)' : 'background:var(--color-background-secondary)'}">
                ...
                <td style="padding:10px;font-size:10px;vertical-align:top;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                  ${control.requiresManualValidation ? `
                    <span style="display:inline-block;background:#FFA500;color:white;padding:3px 8px;border-radius:3px;font-weight:600">Manual Validation</span>
                    ${['super', 'admin'].includes(currentUser?.role) ?
                      (manualValidationApprovals[control.controlId] ?
                        `<span style="display:inline-block;background:var(--clr-success-bg);color:var(--clr-success-text);padding:3px 8px;border-radius:3px;font-weight:600;font-size:9px">✓ Approved</span>` :
                        `<button class="approve-control-btn" data-control-id="${control.controlId}" data-category-id="${currentCategory}" style="padding:3px 8px;border-radius:3px;border:1px solid var(--clr-primary);background:transparent;color:var(--clr-primary);font-weight:600;cursor:pointer;font-size:9px">Approve</button>`
                      ) : ''
                    }
                  ` : `<span style="display:inline-block;background:var(--clr-success-bg);color:var(--clr-success-text);padding:3px 8px;border-radius:3px;font-weight:600">Automatic</span>`}
                </td>
  ...
  setTimeout(() => {
    const approveButtons = el.querySelectorAll('.approve-control-btn')
    approveButtons.forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          const response = await callAPI(`/cap/controls/${controlId}/approve`, 'POST', {
            categoryId,
            status: 'Passed',
            comment: 'Manual validation approved by super admin'
          })

          if (response.success) {
            // Update local approvals cache
            manualValidationApprovals[controlId] = response.data

            // Re-render the table
            renderCategoryAssessmentTab(el, dashboardData.home)
            showToast(`Control ${controlId} approved successfully`, 'success')
          } else {
            showToast(`Failed to approve control: ${response.message}`, 'error')
          }
        } catch (error) {
          console.error('Error approving control:', error)
          showToast(`Error approving control: ${error.message}`, 'error')
        }
      })
    })
  }, 0)
