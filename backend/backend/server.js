...
// Approve manual validation control (super admin only)
app.post('/api/cap/controls/:controlId/approve', (req, res) => {
  try {
    ...
    const { controlId } = req.params
    const { categoryId, status, comment } = req.body

    if (!controlId || !categoryId) {
      return res.status(400).json({ success: false, message: 'controlId and categoryId are required' })
    }

    // Store manual validation approval
    const approvalData = {
      controlId,
      categoryId,
      status: status || 'Passed',
      approvedBy: user.id || user.upn,
      approvedAt: new Date().toISOString(),
      comment: comment || ''
    }

    // Load approvals from file
    const approvalsFile = './data/cap-manual-validations.json'
    let approvals = []
    try {
      const data = fs.readFileSync(approvalsFile, 'utf-8')
      approvals = JSON.parse(data)
    } catch (e) {
      approvals = []
    }

    // Remove old approval for this control if exists
    approvals = approvals.filter(a => a.controlId !== controlId)
    ...
    res.json({
      success: true,
      data: approvalData,
      message: `Control ${controlId} approved by ${user.id || user.upn}`
    })
  } catch (error) {
...
// Get manual validation approvals
app.get('/api/cap/controls/approvals/:categoryId', (req, res) => {
  try {
    ...
    // Filter approvals for this category
    const categoryApprovals = approvals.filter(a => a.categoryId === categoryId)

    res.json({
      success: true,
      data: categoryApprovals
    })
  } catch (error) {
...
app.get('/api/cap/dashboard/drift', async (req, res) => {
  try {
    // Load real policies from Graph API or mock data
    const realPolicies = await loadPolicies()

    // Define baseline/expected configuration for each policy
    ...
    // Process policies and detect drift
