...
function render(el) {
  ...
  el.innerHTML = `
    ...
    <!-- Recent Admin Consents (top-level, before tabs) -->
    ${recentConsents.length > 0 ? `
      <div style="margin-bottom:16px">
        <div class="alert-banner warning mb-3" style="display:flex;align-items:center;justify-content:space-between">
          ...
          <button style="background:none;border:none;cursor:pointer;font-size:16px;padding:0 8px" id="dismiss-recent-consents-header">✕</button>
        </div>
...
function wireSection(el) {
  ...
  content.querySelector('#dismiss-recent-consents-header')?.addEventListener('click', () => {
    const banner = content.querySelector('.alert-banner.warning')
    if (banner) banner.closest('div[style*="margin-bottom"]').style.display = 'none'
  })
